...
import { arrayMoveImmutable } from '../../common/hepler/array-move';
import SortableList, { SortableItem, SortableKnob } from 'react-easy-sort';
import DragKnob from './DragKnob/DragKnob';
...
export const NewTab = () => {
	...
	const onSortEnd = (oldIndex: number, newIndex: number) => {
		setCustomList((array) => {
			const newArr = arrayMoveImmutable(array, oldIndex, newIndex);
			const newMap: Record<string, ShortcutProps> = {};
			newArr.forEach((c) => {
				newMap[c.id!] = c;
			});
			LS.setItem(STORAGE_KEYS.SHORTCUT, newMap);
			return newArr;
		});
	};
			...
			<div className='tab-container'>
				...
				<div className='flex'>
					...
					<div className='container mx-auto'>
						<FixedShortcut />
						<SortableList onSortEnd={onSortEnd} allowDrag={!edit} draggedItemClassName='dragged' className='grid grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-9 2xl:grid-cols-10 mt-4 xl:mt-6 gap-4 xl:gap-6 duration-200'>
							{customList.map((c) => (
								<SortableItem key={c.id!}>
									<div className={`relative cursor-pointer${edit ? ' is-editing' : ''}`}>
										<SortableKnob>{edit ? <></> : <DragKnob />}</SortableKnob>
										{edit ? (
											<div className='edit-mode'>
												<div className='edit-item' onClick={() => editItem(c)}>
													<EditOutlined style={{ fontSize: '18px' }} />
												</div>
												<div className='edit-item' onClick={() => deleteItem(c.id!)}>
                      ...
                      target='_self'
											...
											key={c.id!}
										>
											<div className='max-w-[60px] w-[60px] aspect-square flex items-center overflow-hidden rounded relative img-overlay'>
